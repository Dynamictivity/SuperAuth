 ...
 */
	function startup(&$controller) {
		$this->__checkCookie(); // see if user has the RememberMe cookie
		$isErrorOrTests = (
		...
		switch ($type) {
			...
			case 'acl':
				...
				if (!isset($this->actionMap[$this->params['action']])) {
					...
					if (!$this->isAuthorized('crud', null, $user) || empty($object->id)) {
 ...
 */
	function logout() {
		...
		$this->__deleteCookie(); // delete RememberMe cookie
		$this->_loggedIn = false;
...
/**
 * Methods added by RememberMe
 * Not documented yet
 */
	function __remember($username, $password) {
        $cookie = array();
        $cookie[$this->fields['username']] = $username;
        $cookie[$this->fields['password']] = $password;
